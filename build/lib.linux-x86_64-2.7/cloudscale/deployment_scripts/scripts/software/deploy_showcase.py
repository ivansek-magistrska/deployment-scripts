...
class DeploySoftware:
    ...
    def write_db_config(self, ssh, path):
        ...
        if int(self.props.rds_num_replicas) > 1:
            ...
            cfg += 'jdbc.url=jdbc:mysql://%s/%s?autoReconnect=true\n' % (self.props.cfg.get('platform', 'urls'), self.props.database_name)
    ...
    def unix_shell(self, ip_address):
        try:
            ...
            _, stdout, _ = ssh.exec_command('sudo rm -rf /home/%s/*' % self.props.remote_user)
            self.wait_for_command(stdout)
            ...
            # replace static files url
            _, stdout, _ = ssh.exec_command("sudo sed -i 's/.*eu\.cloudscale\.files\.url\.css.*/eu\.cloudscale\.files\.url\.css=%s\/css/g' %s" % (self.props.static_files_url, 'showcase-1-a/WEB-INF/classes/app.properties'))
            self.wait_for_command(stdout)

            _, stdout, _ = ssh.exec_command("sudo sed -i 's/.*eu\.cloudscale\.files\.url\.img.*/eu\.cloudscale\.files\.url\.img=%s\/img/g' %s" % (self.props.static_files_url, 'showcase-1-a/WEB-INF/classes/app.properties'))
            self.wait_for_command(stdout)

            _, stdout, _ = ssh.exec_command("sudo sed -i 's/.*eu\.cloudscale\.files\.url\.js.*/eu\.cloudscale\.files\.url\.js=%s\/js/g' %s" % (self.props.static_files_url, 'showcase-1-a/WEB-INF/classes/app.properties'))
            self.wait_for_command(stdout)
